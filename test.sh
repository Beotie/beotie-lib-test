...
runner "$PHP_PATH vendor/bin/phpcbf --standard=./csruleset.xml Beotie/LibTest/"
echo "$TEST_RES" >> doc/phpcbf.txt

test "$PHP_PATH vendor/bin/phpunit" PHPUnit 100
echo "$TEST_RES" > doc/phpunit.txt

test "$PHP_PATH vendor/bin/phpcs --standard=./csruleset.xml Beotie/LibTest/" PHPCS 100
echo "$TEST_RES" > doc/phpcs.txt

test "$COMPOSER_PATH validate" COMPOSER 100
echo "$TEST_RES" > doc/composer.txt

test "$PHP_PATH vendor/bin/phpmd Beotie/LibTest/ text ./phpmd.xml" PHPMD 100
echo "$TEST_RES" > doc/phpmd.txt

test "$PHP_PATH vendor/bin/phpcpd Beotie/LibTest/" PHPCPD 1
